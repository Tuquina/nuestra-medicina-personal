...
func TestCreateFailsClosedWhenPaymentsAreNotConfigured(t *testing.T) {
	t.Parallel()
	repository := &repositoryStub{}
	service := NewService(bookCatalogStub{}, repository, &providerStub{})
...
func TestCreateRejectsDraftOrMissingBook(t *testing.T) {
	t.Parallel()
	service := NewService(bookCatalogStub{err: book.ErrNotFound}, &repositoryStub{}, &providerStub{configured: true})
...
func TestProcessPaymentUsesProviderAsIndependentSource(t *testing.T) {
	...
	repository := &repositoryStub{}
	service := NewService(bookCatalogStub{}, repository, &providerStub{configured: true, payment: payment})
