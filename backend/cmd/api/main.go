...
func run(logger *slog.Logger) error {
	...
	authService := authentication.NewService(googleProvider, authRepository, cfg.AdminGoogleSubs, cfg.SessionTTL)
	authorizer := postgres.NewSessionAuthorizer(pool, cfg.AdminGoogleSubs)
	orderRepository := postgres.NewOrderRepository(pool)
	mercadoPagoClient := mercadopago.NewClient(cfg.MercadoPagoToken, cfg.MercadoPagoPublicBaseURL, cfg.MercadoPagoAPIBaseURL)
	...
	backofficeService := backoffice.NewService(backofficeRepository, cfg.AdminGoogleSubs)
